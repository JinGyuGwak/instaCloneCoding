    ...
    public void accessRequest(Constant.SocialLoginType socialLoginType) throws IOException {
        ...
        switch (socialLoginType){ //각 소셜 로그인을 요청하면 소셜로그인 페이지로 리다이렉트 해주는 프로세스이다.
            case GOOGLE:{
                ...
                //getOauthRedirectURL 메소드는 Redirect 처리를 할 URL을 생성하는 메서드이다.
            }break;
            case NAVER:{
        ...
        response.sendRedirect(redirectURL); //해당 소셜로그인 창으로 리다이렉트
    }
            ...
            case GOOGLE: {
                ...
                if(userService.checkUserByEmail(googleUser.getEmail())) { // user가 DB에 있다면, 로그인 진행
                    ...
                    GetSocialOAuthRes getSocialOAuthRes = new GetSocialOAuthRes(
                            ...
                            oAuthToken.getAccess_token(),
                            postUserRes.getJwt());
                    return getSocialOAuthRes; //화면에 띄우는 정보들
                }
            }
            default: {
                throw new BaseException(INVALID_OAUTH_TYPE);
