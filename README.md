...
프로젝트의 초기 단계에서는 JPA를 사용해 기본적인 CRUD 기능을 구현하는 것으로 시작했지만, 지속적인 학습을 통해 Spring Security와 JWT를 활용한 인증,인가 기능 적용, AWS와 Docker를 사용한 배포, 그리고 단위 및 통합 테스트 코드 작성까지 프로젝트를 발전시켜왔습니다. 이 과정에서 코드가 수정될 때마다 수동으로 빌드 및 배포를 하는 것이 불편하다고 느끼게 되었고 이를 개선하기 위해 Jenkins를 이용한 CI/CD 파이프라인을 구축하였습니다.

이 경험을 바탕으로 앞으로도 새로운 지식을 프로젝트에 적용하며 지속적으로 성장해 나갈 계획입니다.
